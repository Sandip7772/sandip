...
class WooHelpers {
	public static function get_woo_cat_featured_image() {
		$args = array(
			'taxonomy'   => 'product_cat',
			'hide_empty' => false, // Set to true if you want to exclude empty categories
		);

		$featured_image_collection = array();
		...
		foreach ( $categories as $category ) {
			...
			$featured_image_collection[ $term_id ] = $image;
		}

		return $featured_image_collection;
	}
	...
	public static function get_woo_product_reviews() {
		...
		foreach ( $comments as $comment ) {
			...
			$comment->reviewer_name = $user_data->display_name;
		}
