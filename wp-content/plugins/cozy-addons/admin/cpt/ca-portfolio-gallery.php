	...
	function ca_cpt_portfolio_gallery_init() {
		$args = array(
			'labels'               => array(
				...
				'all_items'         => __( 'All Templates', 'cozy-addons' ),
				'search_items'      => __( 'Search Portfolio Gallery', 'cozy-addons' ),
				'parent_item_colon' => __( 'Parent Portfolio Gallery:', 'cozy-addons' ),
				'not_found'         => __( 'No Portfolio Gallery found.', 'cozy-addons' ),
			),
			'public'               => true,
...
if ( ! function_exists( 'register_ca_portfolio_gallery_custom_fields' ) ) {
	...
	function register_ca_portfolio_gallery_custom_fields() {
		...
		register_meta(
			...
			array(
				'type'         => 'string',
				'description'  => __( 'URL', 'cozy-addons' ),
				'single'       => true,
				'show_in_rest' => true,
			)
		);
...
if ( ! function_exists( 'ca_portfolio_gallery_fields_callback' ) ) {
	...
	function ca_portfolio_gallery_fields_callback( $post ) {
		...
		<label class="components-base-control__label" for="ca_portfolio_gallery_url"><?php echo esc_html__( 'URL', 'cozy-addons' ); ?></label>
...
if ( ! function_exists( 'ca_portfolio_gallery_save_custom_meta' ) ) {
	...
	function ca_portfolio_gallery_save_custom_meta( $post_id ) {
		...
		if ( isset( $_POST['ca_portfolio_gallery_url'] ) && wp_verify_nonce( $_POST['ca_portfolio_url'] ) ) {
			update_post_meta( $post_id, 'ca_portfolio_gallery_url', sanitize_text_field( $_POST['ca_portfolio_gallery_url'] ) );
		}
	}
